...
class SummaryOfSalesPerAreaExport implements FromArray, ShouldAutoSize, WithColumnFormatting, WithHeadings, WithStyles, WithEvents, WithCustomStartCell, WithPreCalculateFormulas, WithTitle {
    ...
    public function title(): string {
        return 'Summary of Sales Per Area';
    }

    public function registerEvents(): array {
        return [
            AfterSheet::class => function(AfterSheet $event) { 
                $event->sheet->mergeCells(sprintf('A1:X1'));
                $event->sheet->mergeCells(sprintf('A2:X2'));

                $event->sheet->mergeCells(sprintf('A4:X4'));
                
                $event->sheet->setCellValue('A1', env('APP_COMPANY'));
                $event->sheet->setCellValue('A2', env('APP_ADDRESS'));
                $event->sheet->setCellValue('A4', 'SUMMARY OF SALES PER AREA - ' . date('F Y', strtotime($this->period)));
                ...
                $event->sheet->setCellValue('V17', '=SUM(V8:V16)')
                        ->getStyle('V17')->getNumberFormat()->setFormatCode('#,##0.00');
                $event->sheet->setCellValue('W17', '=SUM(W8:W16)')
                        ->getStyle('W17')->getNumberFormat()->setFormatCode('#,##0.00');
                $event->sheet->setCellValue('X17', '=SUM(X8:X16)')
                        ->getStyle('X17')->getNumberFormat()->setFormatCode('#,##0.00');
