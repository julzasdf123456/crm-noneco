...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h4>Billed and Unbilled Reports BAPA</h4>
                </div>
            </div>
        </div>
    </section>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card shadow-none">
                <div class="card-body">
                    {!! Form::open(['route' => 'readings.billed-and-unbilled-reports-bapa', 'method' => 'GET']) !!}
                    <div class="row">
                        ...
                        <div class="form-group col-lg-2">
                            <label for="BAPAName">BAPA</label>
                            <select name="BAPAName" id="BAPAName" class="form-control form-control-sm">
                                @foreach ($bapas as $item)
                                    <option value="{{ $item->OrganizationParentAccount }}" {{ isset($_GET['BAPAName']) && $_GET['BAPAName']==$item->OrganizationParentAccount ? 'selected' : '' }}>{{ $item->OrganizationParentAccount }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($readingReport as $item)
                                <tr>
                                    ...
                                    <td>{{ date('M d, Y h:i:s A', strtotime($item->ReadingTimestamp)) }}</td>
    ...
    <script>
        $(document).ready(function() {
            $('#printBtnReport').on('click', function(e) {
                e.preventDefault()
                window.location.href = "{{ url('/readings/print-billed-unbilled-bapa') }}" + "/" + $('#Type').val() + "/" + encodeURI($('#BAPAName').val()) + "/" + $('#ServicePeriod').val() + "/" + $('#Office').val()
            })
        })
    </script>
@endpush