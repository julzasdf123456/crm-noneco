...
<div  class="content" style="margin: 15px;">
    <div class="row">
        ...
        @if ($memberConsumer != null)
            {{-- IF MEMBER IS JURIDICAL --}}
            @if ($memberConsumer->MembershipType == MemberConsumers::getJuridicalId())
                <div class="col-sm-6">
                    <p class="left-indent">Firm: <u><strong>{{ $serviceConnection->ServiceAccountName }}</strong></u></p>
                    <p class="left-indent">Address: <u><strong>{{ strtoupper(MemberConsumers::getAddress($memberConsumer)) }}</strong></u></p>
                    <p class="left-indent">Residence Cert. No: <u><strong>{{ $serviceConnection->ResidenceNumber }}</strong></u></p>
                </div>   
                <div class="col-sm-6">
                    <p class="left-indent">Representative: <u><strong>{{ $memberConsumer->OrganizationRepresentative }}</strong></u></p>
                    <p class="left-indent">Age: <u><strong>{{ $memberConsumer->Birthdate != null ? MemberConsumers::getAge($memberConsumer->Birthdate) : '-' }}</strong></u></p>
                    <p class="left-indent">Date of Membership: <u><strong>{{ date('F d, Y', strtotime($memberConsumer->DateApplied)) }}</strong></u></p>
                </div>                  
            @else 
                ...
                <div class="col-sm-6">
                    ...
                    <p class="left-indent">Address: <u><strong>{{ strtoupper(MemberConsumers::getAddress($memberConsumer)) }}</strong></u></p>
                    <p class="left-indent">Age: <u><strong>{{ $memberConsumer->Birthdate != null ? MemberConsumers::getAge($memberConsumer->Birthdate) : '-' }}</strong></u></p>
                    <p class="left-indent">Gender: <u><strong>{{ $memberConsumer->Gender }}</strong></u></p>
                    <p class="left-indent">Civil Status: <u><strong>{{ $memberConsumer->CivilStatus }}</strong></u></p>
                    <p class="left-indent">Contact No: <u><strong>{{ $memberConsumer->ContactNumbers }}</strong></u></p>
                    <p class="left-indent">Date of Membership: <u><strong>{{ date('F d, Y', strtotime($memberConsumer->DateApplied)) }}</strong></u></p>
                </div> 
                @if ($spouse != null)
                    <div class="col-sm-6">
                        <p class="left-indent">Spouse: <u><strong>{{ $spouse->FirstName . ' ' . $spouse->LastName . ' ' . $spouse->Suffix }}</strong></u></p>
                        <p class="left-indent">Age: <u><strong>{{ $spouse->Birthdate != null ? MemberConsumers::getAge($spouse->Birthdate) : '-' }}</strong></u></p>
                        <p class="left-indent">Gender: <u><strong>{{ $spouse->Gender }}</strong></u></p>
                        <p class="left-indent">Contact No: <u><strong>{{ $spouse->ContactNumbers }}</strong></u></p>
                    </div>  
                @else
            ...
            <div class="col-sm-12">
                ...
                <p class="left-indent">Building Location: <u><strong>{{ strtoupper(ServiceConnections::getAddress($serviceConnection)) }}</strong></u></p>
                <p class="left-indent">Type of Occupancy: <strong>{{ $serviceConnection->TypeOfOccupancy }}</strong></p>
            </div>

            <div class="col-sm-6">
                <p class="left-indent">Owner of Building: <u><strong>{{ $memberConsumer->MembershipType == MemberConsumers::getJuridicalId() ? (MemberConsumers::serializeSpouse($memberConsumer) != null ? MemberConsumers::serializeSpouse($memberConsumer) : '____________________________________') : MemberConsumers::serializeMemberNameFormal($memberConsumer) }}</strong></u></p> 
                <p class="left-indent">Owner Address: <u><strong>{{ strtoupper(ServiceConnections::getAddress($serviceConnection)) }}</strong></u></p>
                <p class="left-indent">Residence Cert. No: <u><strong>{{ $serviceConnection->ResidenceNumber }}</strong></u></p>
            </div>
            <div class="col-sm-6">
            ...
            <div class="col-sm-12">
                ...
                <p><strong>IV. TYPE OF MEMBERSHIP: <u>{{ $memberConsumer->Type }}</u></strong></p>
            </div>

            <div class="col-sm-3 offset-sm-8">
                <p class="text-center"><strong>{{ $memberConsumer->MembershipType == MemberConsumers::getJuridicalId() ? MemberConsumers::serializeSpouse($memberConsumer) : MemberConsumers::serializeMemberNameFormal($memberConsumer) }}</strong></p>
                <div class="divider"></div>
                <p class="text-center">Applicant's Name Over Signature</p>
            </div>
        @else
            <p><i>No Membership record found!</i></p>
        @endif
