...
<div id="print-area">
    <div style="text-align: center; display: inline;">
        ...
        <h4 class="text-center">STATEMENT OF ACCOUNT FOR NET METERING</h4>
    </div>
    ...
    <div class="half" style="float: left;">
        <table class="bordered" style="width: 100%;">
            ...
            <tr class="no-border-top-bottom">
                <td class="no-border-top-bottom text-left" colspan="5" style="opacity: 0;">TEST</td>
            </tr>
            <tr class="no-border-top-bottom">
        ...
        <table class="bordered" style="width: 100%; margin-top: 2px;">
            ...
            <tr class="no-border-top-bottom">
                <th class="no-border-top-bottom text-left">NET METERING CHARGES</th>
                <th class="no-border-top-bottom"></th>
                <th class="no-border-top-bottom"></th>
            </tr>
            <tr class="no-border-top-bottom">
                <td class="no-border-top-bottom text-left left-indent">Distribution Demand  Charge (Php/kw)</td>
                <td class="no-border-top-bottom text-right">{{ $rate->DistributionDemandCharge }}</td>
                <td class="no-border-top-bottom text-right">{{ number_format($bills->SolarDemandChargeKW, 2) }}</td>
            </tr>
            <tr class="no-border-top-bottom">
                <td class="no-border-top-bottom text-left left-indent">Distribution System Charge (Php/kwh)</td>
                <td class="no-border-top-bottom text-right">{{ $rate->DistributionSystemCharge }}</td>
                <td class="no-border-top-bottom text-right">{{ number_format($bills->SolarDemandChargeKWH, 2) }}</td>
            </tr>
            <tr class="no-border-top-bottom">
                <td class="no-border-top-bottom text-left left-indent">Supply Retail Cust. Charge (Php/cust/mo)</td>
                <td class="no-border-top-bottom text-right">{{ $rate->SupplyRetailCustomerCharge }}</td>
                <td class="no-border-top-bottom text-right">{{ number_format($bills->SolarRetailCustomerCharge, 2) }}</td>
            </tr>
            <tr class="no-border-top-bottom">
                <td class="no-border-top-bottom text-left left-indent">Supply System Charge (Php/kwh)</td>
                <td class="no-border-top-bottom text-right">{{ $rate->SupplySystemCharge }}</td>
                <td class="no-border-top-bottom text-right">{{ number_format($bills->SolarSupplySystemCharge, 2) }}</td>
            </tr>
            <tr class="no-border-top-bottom">
                <td class="no-border-top-bottom text-left left-indent">Metering Retail Cust. Charge (Php/cust/mo)</td>
                <td class="no-border-top-bottom text-right">{{ $rate->MeteringRetailCustomerCharge }}</td>
                <td class="no-border-top-bottom text-right">{{ number_format($bills->SolarMeteringRetailCharge, 2) }}</td>
            </tr>
            <tr class="border-bottom-only">
                <td class="border-bottom-only text-left left-indent">Metering System Charge (Php/kwh)</td>
                <td class="border-bottom-only text-right">{{ $rate->MeteringSystemCharge }}</td>
                <td class="border-bottom-only text-right">{{ number_format($bills->SolarMeteringSystemCharge, 2) }}</td>
            </tr>
        </table>
    </div>
    ...
    <div class="half" style="float-right; margin-left: 5px;">
        <table class="bordered" style="width: 100%;">
            ...
            <tr>
                ...
                <th colspan="2" class="text-center">Energy Import</th>
                <th></th>
                <th></th>
            </tr>
            <tr>
                <th></th>
                <th></th>
                <th class="text-center">{{ is_numeric($bills->PresentKwh) ? round(floatval($bills->PresentKwh), 2) : $bills->PresentKwh }}</th>
                <th class="text-center">{{ is_numeric($bills->PreviousKwh) ? round(floatval($bills->PreviousKwh), 2) : $bills->PreviousKwh }}</th>
                ...
                <th class="text-center">{{ is_numeric($bills->KwhUsed) ? round(floatval($bills->KwhUsed), 2) : $bills->KwhUsed }}</th>
            </tr>
            <tr>
                <th class="text-center"></th>
                <th class="text-center"></th>
                <th colspan="2" class="text-center">Energy Export</th>
                <th></th>
                <th></th>
            </tr>
            <tr>
                <th></th>
                <th></th>
                <th class="text-center">{{ is_numeric($bills->SolarExportPresent) ? round(floatval($bills->SolarExportPresent), 2) : $bills->SolarExportPresent }}</th>
                <th class="text-center">{{ is_numeric($bills->SolarExportPrevious) ? round(floatval($bills->SolarExportPrevious), 2) : $bills->SolarExportPrevious }}</th>
                <th class="text-center">{{ round((is_numeric($bills->SolarExportPresent) ? round(floatval($bills->SolarExportPresent), 2) : 0) - (is_numeric($bills->SolarExportPrevious) ? round(floatval($bills->SolarExportPrevious), 2) : 0), 2) }}</th>
                <th class="text-center">{{ is_numeric($bills->SolarExportKwh) ? round(floatval($bills->SolarExportKwh),2) : '-' }}</th>
            </tr>
            <tr>
                <td colspan="6">DEMAND: <strong>{{ is_numeric($bills->DemandPresentKwh) ? round(floatval($bills->DemandPresentKwh), 2) : $bills->DemandPresentKwh }}</strong></td>
            </tr>
        </table>
        ...
        <table class="bordered" style="width: 100%; margin-top: 2px;">
            ...
            <tr class="border-bottom-only">
                <td class="border-bottom-only text-left left-indent">Katas Ng VAT</td>
                <td class="border-bottom-only text-right"></td>
                <td class="border-bottom-only text-right">-{{ number_format($bills->KatasNgVat, 2) }}</td>
            </tr>
            {{-- NET METERING --}}
            <tr class="no-border-top-bottom">
                <th class="no-border-top-bottom text-left">CUSTOMER CHARGES TO DISTRIBUTION UTILITY</th>
                <td class="no-border-top-bottom text-right"></td>
                <td class="no-border-top-bottom text-right"></td>
            </tr>
            <tr class="no-border-top-bottom">
                <td class="no-border-top-bottom text-left left-indent">Generation Charge (Net Export)</td>
                <td class="no-border-top-bottom text-right">{{ $rate->GenerationSystemCharge }}</td>
                <th class="no-border-top-bottom text-right">{{ number_format($bills->GenerationChargeSolarExport, 2) }}</th>
            </tr>
            <tr class="border-bottom-only">
                <td class="border-bottom-only text-left left-indent">Residual Credit Earned in Prior Months</td>
                <td class="border-bottom-only text-right"></td>
                <th class="border-bottom-only text-right">{{ number_format(floatval($bills->Item4) - floatval($bills->GenerationChargeSolarExport), 2) }}</th>
            </tr>

            <tr class="no-border-top-bottom">
                <th class="no-border-top-bottom text-left left-indent">CURRENT AMOUNT DU TO CUSTOMER</th>
                <td class="no-border-top-bottom text-right"></td>
                <th class="no-border-top-bottom text-right">{{ number_format($bills->Item1, 2) }}</th>
            </tr>
            <tr class="no-border-top-bottom">
                <th class="no-border-top-bottom text-left left-indent">CURRENT AMOUNT CUSTOMER TO DU</th>
                <td class="no-border-top-bottom text-right"></td>
                <th class="no-border-top-bottom text-right">{{ number_format($bills->Item4, 2) }}</th>
            </tr>
