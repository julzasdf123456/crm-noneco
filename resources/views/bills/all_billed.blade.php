...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h4>All Billed Reports | Per Area, Per Consumer Type</h4>
                </div>
            </div>
        </div>
    </section>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card shadow-none">
                <div class="card-body">
                    {!! Form::open(['route' => 'bills.all-billed', 'method' => 'GET']) !!}
                    <div class="row">
                        ...
                        <div class="form-group col-lg-2">
                            ...
                            <select name="AccountType" id="AccountType" class="form-control form-control-sm">
                                ...
                                @foreach ($accountTypes as $item)
                                    <option value="{{ $item->AccountType }}" {{ isset($_GET['AccountType']) && $_GET['AccountType']==$item->AccountType ? 'selected' : '' }}>{{ $item->AccountType }}</option>
                                @endforeach
                            </select>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('#download').on('click', function(e) {
                e.preventDefault()
                window.location.href = "{{ url('/bills/download-all-billed') }}" + "/" + $('#Town').val()  + "/" + $('#ServicePeriod').val() + "/" + encodeURIComponent($('#AccountType').val())
            })
        })
    </script>
@endpush